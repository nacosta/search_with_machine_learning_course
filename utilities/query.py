...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", query_classifier: bool=False):
    #### W3: classify the query
    #### W3: create filters and boosts
    # Note: you may also want to modify the `create_query` method above

    max_predicted_cats = 3
    category_score_threshold = 0.5

    global query_model
    if query_classifier and query_model is None:
        query_model = fasttext.load_model("/workspace/datasets/fasttext/model_week3.bin")

    filters = None
    if query_classifier:
        predicted_categories, scores = query_model.predict(user_query, 3)
        for p,s in zip(predicted_categories, scores):
            print(f"{p} -> {s}")

        filters = {'terms': {'categoryPathIds': []}}
        total_score = 0
        for i, score in enumerate(scores):
            filters['terms']['categoryPathIds'].append(predicted_categories[i].replace('__label__', ''))
            if (total_score:= total_score + score) >= category_score_threshold:
                break

        if total_score < category_score_threshold:
            # discard filters if total_score below threshold
            filters = None  
...
if __name__ == "__main__":
    ...
    general.add_argument('--query_classifier', default=False, action='store_true',
                         help='Filters using predicted category')
    ...
    print(query_prompt)
    while (query:= input()).lower() !=  "exit":
        query = query.rstrip()
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, query_classifier=query_classifier)
